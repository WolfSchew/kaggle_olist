...
class Seller:
    '''
        ...
        sellers as index and various properties of 
        these sellers as columns
    '''

    def __init__(self):
        # Import data only once
        self.data = get_data()
        self.matching_table = get_matching_table()
        self.order = Order()
    ...
    def get_seller_delay_wait_time(self):
        """
        Returns a DataFrame with with the columns:
        ...
        def delay_to_logistic_partner(df):
            df['delay'] = (df.shipping_limit_date -
                      df.order_delivered_carrier_date) / np.timedelta64(1, 'D')
...
if __name__ == '__main__':
    sellers = Seller()
    data = sellers.get_training_data()
    print(data.shape)
    print(data.head(2))
